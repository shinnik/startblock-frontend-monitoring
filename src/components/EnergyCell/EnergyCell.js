...
function EnergyCell({name, money, active, ...rest}) {
    return <Paper elevation={active ? 1 : 2} {...rest}>
        <Typography style={{display: active ? 'block' : 'none', fontSize: '32px', paddingTop: '8px', paddingLeft: '10px'}}><b>{name}</b></Typography>
            <Box style={{display: active ? 'block' : 'none',}} className={classes.Box}>
