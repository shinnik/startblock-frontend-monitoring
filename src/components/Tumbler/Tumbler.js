...
import PowerSettingsNew from './power-settings-icon.svg';
...
function Tumbler({ direction, power: state, id, dispatch, koeff, hovered, noHover }) {
    ...
    return <Box style={{ pointerEvents: noHover ? "none" : "auto" }} className={arrowClass(direction, state)}>
        ...
        />
        {hovered &&
            <Box style={{ display: 'flex' }} onClick={handle} >
                <div style={{ backgroundColor: state ? '#EB5757' : '#878787', width: constants.triangleSize, height: constants.triangleSize }} className={powerIconClass(direction)} >
                    {/* <img style={{ backgroundColor: state ? '#EB5757' : '#878787', width: constants.triangleSize, height: constants.triangleSize }} src={PowerSettingsNew} /> */}
                </div>
            </Box>}
    </Box>;
}
